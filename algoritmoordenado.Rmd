...
#Upload your data
#Your data should have at least four colums: first_name, last_name, country_code (Alpha-2) & unique_id
...
dataslav <- dataslav[,-c(7:13)]
colnames(dataslav)[7] <- "Gender"

#Check gender on Slavic names using first_name

dataslavU <- subset(dataslav, !(Gender %in% c("Man", "Woman")))
dataslavFM <- subset(dataslav, (Gender %in% c("Man", "Woman")))

dataslav <- left_join(x = dataslavU, y = WDNAMESrest, by = c("first_name", "country_code"), multiple = "all")

#Eliminate unnecessary columns and rename, so all results have the same column names and order for matching (the extra columns to eliminate will depend on your data)

dataslav <- dataslav[, -c(7:9,11,12)]
colnames(dataslav)[7] <- "Gender"
dataslav <- dataslav %>%
  mutate(Gender = ifelse(is.na(Gender), "Unknown", Gender))

dataslav <- rbind(dataslavFM, dataslav)
...
data_unknown_gender <- data_unknown_gender[, -c(7,9)]
colnames(data_unknown_gender)[7] <- "Gender"
...
#Sssign gender using language code 
...
data_language <- data_language[,-c(8:11)]
data_language <- data_language[,c(1,3,4,5,6,7,2)]
colnames(data_language)[7] = "Gender"
colnames(data_language)[5] = "country_code"
